...
<html>
	...
	<body>
        ...
        function redirectToStripe() {
            try {
                let stripe = Stripe('<?=stripe_publish_key?>');
                stripe.redirectToCheckout({ sessionId: '<?=$_GET['stripe_session_id']?>' }).then(result => {});
            } catch(error) {
                document.querySelector('p').innerHTML = error;
            }
        }
        document.querySelector('.redirect').onclick = event => {
